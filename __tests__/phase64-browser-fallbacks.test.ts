...
describe('Phase 6.4 Browser Fallback Parity', () => {
  ...
  describe('WORKFLOW', () => {
    ...
    it('complete contiguous ordering and minimum stage invariant', async () => {
      ...
      await expect(removeWorkflowStage('ws_2', 'ws_2')).rejects.toThrow('Cannot remove the last active workflow stage');
    });

    it('PUBLICATION role transfer and article reassignment', async () => {
      await removeWorkflowStage('ws_2', 'ws_1'); // ws_2 is PUBLICATION
      const stages = getStoredWorkflowStages();
  ...
  describe('CATEGORIES', () => {
    ...
    it('standard category protection', async () => {
      ...
      await expect(removeCategory('cat_standard')).rejects.toThrow('Cannot remove standard category');
    });

    it('safe custom category reassignment', async () => {
      await expect(removeCategory('cat_custom')).rejects.toThrow('Must provide a target category id for reassignment');
      
      await removeCategory('cat_custom', 'cat_standard');
      const cats = getStoredCategories();
      const catCustom = cats.find(c => c.id === 'cat_custom');
      expect(catCustom?.isActive).toBe(false);
